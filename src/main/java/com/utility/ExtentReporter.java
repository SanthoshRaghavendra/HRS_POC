...
public class ExtentReporter implements ITestListener {
	...
	public synchronized void onTestSuccess(ITestResult result) {
		...
		try {
			switch(Reporter.getCurrentTestResult().getTestContext().getCurrentXmlTest().getSuite().getName()) {
			case "Web":
				screencapture();
				break;
	...
	public synchronized static void setScreenshotSource() {
		if (getPlatformFromtools().equals("Web")) {
			...
			src = ((TakesScreenshot) getDriver()).getScreenshotAs(org.openqa.selenium.OutputType.FILE);
		}else if(getPlatformFromtools().equalsIgnoreCase("getPlatformFromtools()")) {
